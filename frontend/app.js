...
function sqlAgent() {
    return {
        ...
        async runBenchmark() {
            this.systemMessage = "Running benchmark...";
            try {
                const res = await fetch(BACKEND_URL + "/test/benchmark", { method: "POST" });
                const data = await res.json();
                this.systemMessage = `Benchmark: ${data.successful_tests}/${data.total_tests} passed (${data.success_rate})`;
            } catch (e) {
                this.systemMessage = "Benchmark failed.";
            }
        },
